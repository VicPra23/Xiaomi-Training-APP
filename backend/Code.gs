...
function getReportsHistory(p) {
  try {
    ...
    for (let i = d.length - 1; i >= 1; i--) {
      ...
      // 2. Filtro por Semana
      if (weekFilter) {
        if (getWeekNumber(dO) !== weekFilter) continue;
      }
      
      // 3. Filtro por Mes
      if (monthFilter && monthFilter !== "Todos") {
        if (mNames[dO.getMonth()] !== monthFilter) continue;
      }
      
      // 4. Filtro por Cuenta
      if (accountFilter && accountFilter !== "Todos") {
        if ((d[i][3]||"").toString().trim() !== accountFilter) continue;
      }
      
      // 5. Filtro por Metodología
      if (methodologyFilter && methodologyFilter !== "Todos") {
        if ((d[i][5]||"").toString().trim() !== methodologyFilter) continue;
      }
      ...
      // 7. Buscador por texto (q) - Fecha, Cuenta, Tienda, Modelos, Comentarios
      if (query) {
        const rowDataStr = [
          Utilities.formatDate(dO, Session.getScriptTimeZone(), "dd/MM/yyyy"),
          d[i][3], // Cuenta
          d[i][10], // Tienda (Col 11)
          d[i][14], // Moviles
          d[i][15], // Eco
          d[i][16]  // Comentarios
        ].join(" ").toLowerCase();
        if (rowDataStr.indexOf(query) === -1) continue;
      }
